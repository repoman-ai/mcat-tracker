...
globalThis.window = { location: { search: "?today=2026-09-10" } };
...
assert.match(html, /Deeper review may exceed the budget/);
assert.equal(data.plan.question_targets.uworld_baseline, 216);
assert.equal(data.plan.question_targets.section_bank, 360);
assert.equal(data.plan.weeks.reduce((s, w) => s + w.planned_hours, 0), 367);
...
for (const w of [8, 9, 10, 11]) {
  ...
  assert.equal(rows.reduce((s,r) => s + Number(r.practiceTarget.match(/(\d+) [BC]\/[^ ] Section Bank/)?.[1] || 0), 0), 40);
  assert.equal(rows.filter(r=>r.isSectionBank).length, 2);
  assert.ok(rows.every(r=>r.estimatedWorkload.highMinutes < 400));
  assert.ok(rows.filter(r=>r.isSectionBank).every(r=>/review practice before adding volume/.test(r.sourceNotes)));
}
...
assert.equal(data.sectionBanks.flatMap(b=>b.assignments).map(a=>a.date).sort().at(-1), "2027-01-05");
...
for (const date of ["2027-01-15", "2027-01-19", "2027-01-20", "2027-01-21"]) {
