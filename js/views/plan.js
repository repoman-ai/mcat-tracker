import { scheduledWeekForDate } from "../data.js";
import { escapeAttr, escapeHTML, formatDate, todayISO } from "../utils.js";
import { assignmentDetailHTML, bindAssignmentDetail, statusLabel } from "./shared.js";
...
function rowMatches(row, state, currentWeek) {
...
function dayCard(row, context, forceOpen) {
  const daily = context.state.daily[row.id] || {};
  const status = daily.status || "not-started";
  return `
    <details class="plan-day plan-day--${escapeAttr(row.dayType)}" data-assignment-details="${escapeAttr(row.id)}" ${forceOpen ? "open" : ""}>
      <summary>
        ...
        <div class="plan-day__main"><div class="plan-day__meta"><span>${escapeHTML(row.resource || row.dayType.replaceAll("-", " "))}</span>${row.chapterIds.length ? `<span>${escapeHTML(row.chapterIds.join(" · "))}</span>` : ""}</div><h4>${escapeHTML(row.assignment)}</h4><p>${escapeHTML(row.practiceTargetDisplay || (row.isRest ? "Protected rest" : "No practice quota"))}</p></div>
        <span class="status-badge status-badge--${escapeAttr(status)}">${escapeHTML(statusLabel(status))}</span>
        <span class="disclosure-icon" aria-hidden="true">⌄</span>
      </summary>
      <div class="plan-day__detail">
        ${assignmentDetailHTML(row, context.data, context.state)}
        <div class="button-row"><button class="button button--primary" type="button" data-log-from-plan="${escapeAttr(row.id)}">Log question from this assignment</button>${status !== "complete" && !row.isRest ? `<button class="button button--success" type="button" data-plan-complete="${escapeAttr(row.id)}">Mark complete</button>` : ""}</div>
      </div>
    </details>`;
}

function weekCard(week, rows, context, currentWeek, detail) {
  const matching = rows.filter((row) => rowMatches(row, context.state, currentWeek));
...
export function renderPlan(context, route) {
  ...
  const weeksHTML = context.data.plan.weeks.map((week) => weekCard(week, context.data.schedule.filter((row) => row.week === week.week), context, currentWeek, route.detail)).join("");
...
export function bindPlan(container, context) {
  ...
  container.querySelectorAll("[data-plan-filter]").forEach((select) => {
    select.addEventListener("change", () => { filters[select.dataset.planFilter] = select.value; context.rerender(); });
  });
  container.querySelector("[data-plan-current]")?.addEventListener("change", (event) => { filters.currentWeekOnly = event.currentTarget.checked; context.rerender(); });
  container.querySelector("[data-plan-reset]")?.addEventListener("click", () => { Object.assign(filters, { phase: "all", resource: "all", status: "all", dayType: "all", currentWeekOnly: false }); context.rerender(); });
  ...
  container.querySelectorAll("[data-plan-complete]").forEach((button) => button.addEventListener("click", () => {
    const id = button.dataset.planComplete;
    context.updateState({ ...context.state, daily: { ...context.state.daily, [id]: { ...(context.state.daily[id] || {}), status: "complete", updatedAt: new Date().toISOString() } } });
    context.showToast("Assignment marked complete");
  }));
